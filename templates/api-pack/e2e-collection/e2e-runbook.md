...
```bash
newman run result/<output-slug>/05-postman/collection.json   --environment result/<output-slug>/05-postman/environments/e2e.local.postman_environment.json.example   --folder "F-01 Auth and Project Setup"   --folder "F-02 Product Catalog"   --folder "F-07 Secret Key Lifecycle"
```

### Full chained run

```bash
newman run result/<output-slug>/05-postman/collection.json   --environment result/<output-slug>/05-postman/environments/e2e.local.postman_environment.json.example   --delay-request 150   --reporters cli,json   --reporter-json-export result/<output-slug>/10-reports/raw/e2e-run.json
```

### Teardown-only run

```bash
newman run result/<output-slug>/05-postman/collection.json   --environment result/<output-slug>/05-postman/environments/e2e.local.postman_environment.json.example   --folder "F-10 Full Teardown"
```

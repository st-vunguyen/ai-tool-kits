---
description: "Maintain the full API testing kit after a new spec or supporting docs arrive: detect impact, refresh stale assets, rerun impacted packs, and publish a maintenance report."
agent: "agent"
tools: ['search', 'edit', 'new', 'todos', 'runSubagent', 'problems', 'changes', 'runCommands', 'runTasks']
---

# Maintain Full API Testing Kit
...
API Testing Maintainer + Change Impact Analyst + Traceability Auditor + Execution Triage Lead.
...
Maintenance must also verify whether response-code coverage, error examples, and status-driven samples drifted from the previous source of truth.
...
- documented response codes, error examples, or status semantics changed
...
| `KIT_ROOT` | Repo root of this kit | Must contain `.github/`, `.claude/`, `templates/`, `scripts/`, and `docs/` |
...
| `OUTPUT_SLUG` | Existing API pack slug | Shared slug such as `<slug>` |
...
| `02_strategy-and-traceability-delta.md` | What changed in review/strategy/traceability, including response-code/status coverage deltas |
| `03_core-pack-maintenance.md` | Collection/env/data/helper/status-sample changes and rationale |
...
- Classify impact into one or more buckets:
  ...
  - response-code / error-example / negative-case changes
...
- impacted collections, env templates, data helpers, status matrices, and specialized packs were refreshed
...
| Core pack | Collection, env, data, helper assets, and status-code coverage artifacts align with the updated API surface |
...
3. Ensure changed auth, error, pagination, and response-code semantics are captured in docs first

### D — Refresh runnable packs

1. Update the core collection pack, env templates, data helpers, and status-driven samples (`07` to `09`)
...
- Root cause is in collection scripts, variable capture, environment setup, helper logic, traceability, workload setup, or local runner wiring
...
- Limit changes to API testing assets, support files, runtime tooling files in scope, and maintenance reports
- Keep all generated artifacts inside `result/` and approved support paths only
...
- **Status coverage delta**: [new/removed/changed response codes, blocked statuses, sample updates]
